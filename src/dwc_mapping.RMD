...
subtitle: "For: Landsnails from Spain"
author:
- Ward Langeraert
- Dimitri Brosens
date: "`r Sys.Date()`"
...
```{r}
input_data <- read_excel(path = here("data", "raw", "NorthernSpain_ES_TOTAL_v1.xlsx")) 
```
...
input_data %<>% select(-c(island, islandGroup))
```
...
input_data %<>% mutate(scientificname = recode(verbatimScientificName,
  "Euglesa cf. casertana" = "Euglesa casertana",
  "Potamopyrgus antipodarum f. carinata" = "Potamopyrgus antipodarum carinata",
  "Euglesa aff. globulare" = "Euglesa globulare"
))
```
...
input_data %<>% mutate(rankmarker = recode(verbatimScientificName,
  "Obscurella spec." = "genus",
  "Alzoniella spec." = "genus",
  "Deroceras spec." = "genus",
  "Vitrea spec." = "genus",
  "Helicella spec." = "genus",
  "Hygromiidae spec." = "family",
  "Gastropoda spec." = "class",
  "Arion (Arion) spec." = "subgenus",
  "Arion (Kobeltia) spec.." = "subgenus",
  .default = rankmarker
))
```
...
```{r}
occurrence %<>% mutate(datasetID = "https://doi.org/10.15468/wfiktk") # e.g. "https://doi.org/10.15468/xvuzfh"
```
